...
EM.MCCR3 <- function(data, train_ratio = 0.7, tol, c, maxit, alpha1, lambda1, alpha2, lambda2, alpha3, lambda3) {
  ...
  valid_split <- training_data[-train_indices, ]
  ...
  while (continue) {
    ...
    minsplit_vals <- c(2, 10, 20, 30)
    cp_vals <- c(0.01, 0.005, 0.001, 0.0001)
    maxdepth_vals <- c(5, 10, 30)
    ...
    for (ms in minsplit_vals) {
      for (cp in cp_vals) {
        for (md in maxdepth_vals) {
          model <- rpart(d ~ ., data = training_data, method = "class",
                         control = rpart.control(minsplit = ms, cp = cp, maxdepth = md))
          
          # Predict on validation set
          pred <- predict(model, newdata = valid_split, type = "class")
          ...
          if (acc > best_accuracy) {
            best_accuracy <- acc
            best_model <- model
            best_params <- list(minsplit = ms, cp = cp, maxdepth = md)
          }
        }
      }
    }
    ...
    # Final prediction on test set using best_model
    pred <- predict(best_model, newdata = test_data, type = "class")
    pred_prob <- predict(best_model, newdata = test_data, type = "prob")
    ...
    # Optional: If you want class "0" predicted probability
    cured_prob <- pred_prob[, "0"]
