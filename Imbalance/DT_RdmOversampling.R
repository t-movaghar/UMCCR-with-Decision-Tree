...
EM.MCCR3 <- function(data, train_ratio = 0.7, tol, c, maxit, alpha1, lambda1, alpha2, lambda2, alpha3, lambda3) {
  ...
  # Oversample function
  oversample_class <- function(df, target_size) {
    df[sample(1:nrow(df), size = target_size, replace = TRUE), ]
  }
  
  # Oversample classes 2 and 3 in the training set only
  training_data_balanced <- bind_rows(
    training_data %>% filter(d == 0),
    training_data %>% filter(d == 1),
    oversample_class(training_data %>% filter(d == 2), max_count),
    oversample_class(training_data %>% filter(d == 3), max_count)
  )
