...
EM.MCCR3 <- function(data, train_ratio = 0.7, tol, c, maxit, alpha1, lambda1, alpha2, lambda2, alpha3, lambda3) {
  ...
  # Ensure 'd' is a factor for BLSMOTE
  training_data$d <- as.factor(training_data$d)
  
  # BLSMOTE requires numeric features; exclude response 'd'
  features <- training_data %>% select(-d)
  labels <- training_data$d
  
  # Apply Borderline-SMOTE for multiclass
  blsmote_result <- BLSMOTE(X = features, target = labels, K = 5)
  
  training_data_balanced <- blsmote_result$data
  names(training_data_balanced)[ncol(training_data_balanced)] <- "d"
  training_data_balanced$d <- as.numeric(as.character(training_data_balanced$d))  # convert back to numeric
