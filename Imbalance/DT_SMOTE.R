...
EM.MCCR3 <- function(data, train_ratio = 0.7, tol, c, maxit, alpha1, lambda1, alpha2, lambda2, alpha3, lambda3) {
  ...
  # Ensure 'd' is a factor for SMOTE
  training_data$d <- as.factor(training_data$d)
  
  # SMOTE requires numeric features; exclude response 'd' and use rest as predictors
  features <- training_data %>% select(-d)
  labels <- training_data$d
  
  # Apply SMOTE for multiclass (only for minority classes 2 and 3)
  smote_result <- SMOTE(X = features, target = labels, K = 5, dup_size = 0)
  
  training_data_balanced <- smote_result$data
  names(training_data_balanced)[ncol(training_data_balanced)] <- "d"
  training_data_balanced$d <- as.numeric(as.character(training_data_balanced$d))  # back to numeric if needed
