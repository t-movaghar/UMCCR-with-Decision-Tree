...
EM.MCCR3 <- function(data, tol, c, maxit, alpha1, lambda1, alpha2, lambda2, alpha3, lambda3) {
  ...
  while (continue) {
    ...
    # DT 
    dt_model <- rpart(d ~ ., data = data, method = "class")
    cured_prob <- predict(dt_model, newdata = data0, type = "prob")[, 2]
